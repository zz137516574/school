  ...
  <div>
    <div id="courseItem">
      <ul class="courseItemUl clearfix">
        <li class="courseItemLi clearfix" v-for="(item,index) in courseData" :key="index" @click="freeEnroll(item)">
          <div class="courseTop">
            <img :src="item.activityPhoto" alt="">
          </div>
          <div class="courseBottom">
            <div class="top">
              <span class="courseTitle">{{item.activityTitle}}</span>
            ...
            <div class="clearfix ">
              <span class="courseTime f-fl">{{item.activityAddtime}}</span>
              <span class="f-fr price"  v-if="item.activityPrice != 0 "><span  style="color: #ff8a0c;">￥{{item.activityPrice | currency}}</span></span>
              <span class="f-fr price"  v-if="item.activityPrice == 0 "  style="color: #ff8a0c;">免费</span>
            </div>
          </div>
        </li>
      </ul>
