...
//      注册
      registerNow() {
        ...
        if(!(/^[a-zA-Z0-9_]{0,}$/.test(this.retrievePassFormRes.username)) && 6 <= this.retrievePassFormRes.username.length <= 15){
          this.$Message.error("用户名不能含有中文或特殊字符，并且长度不能小于6位且不能超过15位");
          return false;
        }
