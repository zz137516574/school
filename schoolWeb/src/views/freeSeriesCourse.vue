    ...
    <div class="freeClass">
      ...
      <breadcrumb></breadcrumb>
        ...
        <div class="container clearfix classHeaderMian">
            <i class="f-fl img" v-if="courseDetailsData">
            ...
            <div class="f-fl main">
              <div class="name clearfix" style="position: relative">
                ...
                <div class="clearfix bottomMetas">
                    ...
                    <!--<button class="f-fr m-l-10" @click="addShareCourse">添加共享课</button>-->
                    <!--<button class="f-fr m-l-10" @click="addCourse">创建普通课</button>-->
            ...
            <Tabs value="name1" class="f-fl">
              ...
              <!--课程目录可进入详情页面-->
                <TabPane label="课程目录" name="name2">
                    <ul class="CourseList">
                        <li v-if="courseCatalogues" v-for="(item,index) in courseCatalogues" :key="index" class="clearfix liList" @click="CourseDetails(item)">
                          <Radio class="f-fl">{{item.courseName}}</Radio>
                          <i class="ivu-icon ivu-icon-android-arrow-dropright-circle f-fr font-s20"></i>
                          <div class="time f-fr">{{item.addTime}}</div>
                        </li>
                    </ul>
                </TabPane>
...
//              学员笔记
            ...
            if(this.msg === '登陆成功！'){
              if(window.localStorage.memberInfo != null && window.localStorage.memberInfo != undefined){
                if(this.price != 0 && this.isBuy !== 1){
                  this.$router.push({ path: 'orderPurchase', query: {seriesCourseId: this.seriesCourseId, onorsome:this.onorsome,roomNo: this.roomNo, vhallID:this.vhallID, vhallAccount:this.vhallAccount,userId:this.userId}});
                }else{
                  let myurl="http://room.goboosoft.com"+"?"+"courseId="+this.seriesCourseId+"&onorsome="+this.onorsome + "&roomNo=" + this.roomNo + "&vhallID=" + this.vhallID + "&vhallAccount=" + this.vhallAccount+'&userId='+this.userId;
          ...
          addCourse(){
            if(this.msg === '登陆成功！'){
              if(window.localStorage.memberInfo != null && window.localStorage.memberInfo != undefined){
                this.$router.push({ path: 'creatingSeriesCour', query: { seriesCourseId: this.seriesCourseId, onorsome:this.onorsome}});
              }else{
                this.$Message.error({
                  content: '请先绑定云联惠授权！ ',
                  duration: 5
          ...
          addShareCourse (){
            if(this.msg === '登陆成功！'){
              if(window.localStorage.memberInfo != null && window.localStorage.memberInfo != undefined){
                this.$router.push({ path: 'addSharedCourse', query: { seriesCourseId: this.seriesCourseId, onorsome:this.onorsome}});
              }else{
                this.$Message.error({
                  content: '请先绑定云联惠授权！ ',
                  duration: 5
...
//          收藏
          collectEvent(){
            if( this.collectName === '收藏' && this.test6){
              console.log(this.type+'===='+this.seriesCourseId+'===='+this.recordType+'===='+this.userId);
              this.service.collegeCourseNotice(this.type, this.seriesCourseId, this.recordType, this.userId).then(result => {
                this.test6 = true;
                if(result.code === 0 ){
                  this.collectName = '已收藏';
              ...
              this.test6 = false;
            }else if( this.collectName ==='已收藏'){
              this.service.collegeCourseDelete(this.type, this.seriesCourseId, this.recordType, this.userId).then(result => {
                this.test6 = true;
                if(result.code === 0 ){
                  this.collectName = '收藏';
...
//          进入课程目录对应的详情页面
          CourseDetails(item){
            this.onorsome = 0;
            this.$router.push({ path: 'freeCourse', query: { onorsome:this.onorsome, courseId:item.courseId, price:item.price}});
          }
        }
    };
</script>
