<template>
    <div id="class">
        ...
        <div class="yun-body clearfix container m-t-20 m-b-20">
            <Tabs value="name1" class="container" style="height: 474px !important;">
                ...
                <TabPane label="直播列表" name="name5" class="yun-zhibo">
                  <div class="container searchVolume clearfix">
                    <Menu mode="horizontal" :active-name="position"  class="f-fl">
                      <MenuItem :name="index" v-for="(item,index) in liveStatesList" :key="index">
                        <span @click="directSeedingType(item)" style="display: inline-block;width: 100%;height: 100%">{{item}}</span>
                      </MenuItem>
                    </Menu>
                  </div>
                  <div class="tabCont">
                      <v-courseItem :courseData="liveBroadcastDate" :courseStatusData="courseOnlineItemStatus" v-on:courseDetailsRefre="liveDetail"></v-courseItem>
                  </div>
                  <!--分页-->
                  <Page :total="pageNums * 10" class="container text-center" @on-change="newsPageChange"></Page> <!---->
                </TabPane >
            </Tabs>
        </div>
...
<script>
    ...
    import courseItem from '../components/course/coursedivItem.vue';
...
//            }
        ...
        methods: {
          ...
          teacherInfoOn(itemId){
            this.$router.push({path: 'assLecturersInfo', query: {lecturersId: itemId.userId}});
          },
          //在教系列课
          ...
          liveNavLists(){
            this.limit = 4;
            var _this = this;
            console.log('page'+this.page+'limit'+this.limit+'schoolId'+this.schoolId+'liveState'+this.liveState);
...
<style lang="less">
    ...
    .yun-body{
        ...
        .yun-shizi,.yun-banji,.yun-kecheng{
            padding-top: 30px;
        }
        .yun-zhibo{
          .ivu-radio-group{
